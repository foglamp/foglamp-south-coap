...
from python.foglamp.plugins.south.coap.coap import CoAPIngest, Ingest, _DEFAULT_CONFIG as config
...
async def test_plugin_start(mocker, unused_port, loop):
    ...
    # WHEN
    loop.call_soon(coap.plugin_start(config))
...
def test_plugin_reconfigure(mocker, unused_port):
    ...
    config['port']['value'] = config['port']['default']
    config['uri']['value'] = config['uri']['default']
    pstop = mocker.patch.object(coap, '_plugin_stop', return_value=True)
    log_info = mocker.patch.object(coap._LOGGER, "info")

    # WHEN
    new_config = coap.plugin_reconfigure(config, _NEW_CONFIG)
...
def test_plugin_reconfigure_else(mocker, unused_port):
    ...
    # WHEN
    new_config = coap.plugin_reconfigure(config, config2)
